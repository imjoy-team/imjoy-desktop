...
const _initializedCallbacks = []
function addInitialized(cb) {
    _initializedCallbacks.push(cb)
}
export async function startImageJ(appContainer, showDialog, makeTray) {
    ...
    window.fileDialog = new A11yDialog(el);
...
window.onImageJInitialized = async () => {
  ...
  for (let cb of _initializedCallbacks) {
    cb();
  }
